...
- Codexのstdout・stderr・終了コード・timeout・記録済みプロセスツリーPID・最終報告JSONを検査する。timeout時はtaskkill結果とstdout/stderrを保存し、記録した親子PIDを再照会する。
- 再作業はIssueコメント中の`LIAISON_REWORK_APPROVED`、対象既存PR番号またはhead SHA、`gm-approved`と`ready-for-codex`の再付与を必須とし、既存PRへ構造化した実行記録を追記する。
