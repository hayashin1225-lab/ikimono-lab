...
function Test-SelfTest($Config) {
  ...
  $probe = Join-Path $Config.StatePath ".probe-$PID.lock"; $created = $false
  try { New-Item -ItemType Directory -Force -Path $Config.StatePath | Out-Null; $created = $true; $stream = New-Object IO.FileStream($probe,[IO.FileMode]::CreateNew,[IO.FileAccess]::ReadWrite,[IO.FileShare]::None); $stream.Dispose() } finally { if(Test-Path $probe){Remove-Item $probe -Force}; if($created -and (Test-Path $Config.StatePath) -and -not (Get-ChildItem -Force $Config.StatePath | Select-Object -First 1)){Remove-Item $Config.StatePath -Force} }
